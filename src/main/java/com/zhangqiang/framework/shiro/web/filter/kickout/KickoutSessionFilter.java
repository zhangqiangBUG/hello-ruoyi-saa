package com.zhangqiang.framework.shiro.web.filter.kickout;
...
public class KickoutSessionFilter extends AccessControlFilter
{
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception
    {
        ...
        try
        {
            ...
            String cachekey = user.getTenantCode()+"_"+loginName;
            Serializable sessionId = session.getId();

            // 读取缓存用户 没有就存入
            Deque<Serializable> deque = cache.get(cachekey);
            if (deque == null)
            {
                // 初始化队列
            ...
            while (deque.size() > maxSession)
            {
                ...
                try
                {
                    ...
                    if (null != kickoutSession)
                    {
                        ...
                        setKickoutUrl("/"+user.getTenantCode()+"/login?kickout=1");
                    }
                }
                catch (Exception e)
                {
    ...
    private boolean isAjaxResponse(ServletRequest request, ServletResponse response) throws IOException
    {
        ...
        {
            WebUtils.issueRedirect(request, response, kickoutUrl,(Map)null, true, false);
        }
        return false;
    }
