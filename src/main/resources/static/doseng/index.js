...
function() {
    if ($(this).width() < 769) {
        ...
        $(".slimScrollDiv").removeClass("overflow")
    }
    SmoothlyMenu();
});
...
$(function() {
    ...
    function menuItem() {
        ...
        // 选项卡菜单不存在
        if (flag) {
            var str = '<a href="javascript:;" class="active menuTab" data-id="' + data_id + '">' + menuName + ' <i class="fa fa-times-circle"></i></a>';
            $('.menuTab').removeClass('active');

            // 添加选项卡对应的iframe
            var str1 = '<iframe class="RuoYi_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + data_id + '" seamless></iframe>';
    ...
    function menuBlank() {
    	// 新窗口打开外网以http://开头，如http://doseng.vip
    	var dataUrl = $(this).attr('href');
    	window.open(dataUrl);
    	return false;
    }
    ...
    function activeTab() {
        if (!$(this).hasClass('active')) {
            ...
            // 显示tab对应的内容区
            $('.mainContent .RuoYi_iframe').each(function() {
                if ($(this).data('id').indexOf(currentId)!=-1) {
                    $(this).show().siblings('.RuoYi_iframe').hide();
                    return false;
                }
            });
