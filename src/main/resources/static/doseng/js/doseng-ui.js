...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            init: function (options) {
                var defaults = {
                    ...
                    async: true,
                    height: $('#bootstrap-table').offsetParent().outerHeight()-30,
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    async: options.async,                               // 20200814172706 conmi新增异步请求
            ...
            // 搜索-默认第一个form
            search: function (formId, tableId, data, needAsync) {
                table.set(tableId);
            ...
            exportExcel: function (formId) {
                var prompt = "确定导出所有" + table.options.modalName + "吗？";
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                if (id.length > 0) {
                    prompt = "确定导出选中的" + table.options.modalName + "吗？";
                }
                ...
                $.modal.confirm(
                    ...
                    function () {
                        ...
                        dataParam.push({"name": "ids", "value": id});
                        $.modal.loading("正在导出数据，请稍后...");
                        $.post(
                            table.options.exportUrl,
                            dataParam,
                            function (result) {
        ...
        operate: {
            ...
            addTab: function (id) {
                table.set();
                $.modal.openTab("新增" + table.options.modalName, $.operate.addUrl(id));
            },
            // 新增信息 全屏
            addFull: function (id) {
                table.set();
                var url = $.common.isEmpty(id) ? table.options.createUrl : table.options.createUrl.replace("{id}", id);
                $.modal.openFull("新增" + table.options.modalName, url);
            },
            // 新增访问地址
            addUrl: function (id) {
            ...
            // 修改信息
            edit: function (id) {
                // ------------------- begin：针对‘IE9’的选中多行并修改的问题 -------------------
                if (typeof id === 'undefined' && navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE9.0") {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            ...
            ajaxSuccess: function (result) {
                if (result.code == web_status.SUCCESS && table == undefined) {
                    $.modal.msgSuccess(result.msg)
                } else if (result.code == web_status.SUCCESS && table.options == undefined) {
                    $.modal.msgSuccess(result.msg)
                } else if (result.code == web_status.SUCCESS && table.options.type == undefined) {
                    $.modal.msgSuccess(result.msg)
                } else if (result.code == web_status.SUCCESS && table.options.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.msg);
                    $.table.refresh();
...
/**密码显示隐藏**/
function psw_eye(element, name) {
    // 避免IE9的显示问题
    if (navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.split(";")[1].replace(/[ ]/g, "") == "MSIE9.0") {
        if ($('#' + name).attr("type") === "text" && $('#' + name).val() === $('#' + name).attr("placeholder")) {
            return;
        }
    }

    if (element.getAttribute('Class') == "fa fa-eye-slash") {
        $(element).removeClass("fa fa-eye-slash").addClass("fa fa-eye");
        // $(element).prev().attr("type", "text");
        $('#' + name).attr("type", "text");
    } else {
        $(element).removeClass("fa fa-eye").addClass("fa fa-eye-slash");
